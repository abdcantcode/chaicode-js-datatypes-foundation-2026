...
export function extractRangoliCenter(design, start, end) {
  if(typeof design!=='string'||typeof start!=='number'||typeof end!=='number'||Number.isNaN(start)|| Number.isNaN(end)) return "";
...
export function makeRangoliBorder(char, length) {
  if(typeof char!=='string'|| typeof length!=='number' || length<0 || Number.isNaN(length)) return ""
