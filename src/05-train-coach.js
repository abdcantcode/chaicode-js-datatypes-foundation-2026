...
export function findPassenger(passengers, name) {
  ...
  if(!Array.isArray(passengers)||typeof name!=='string'||name.trim().length===0) return undefined;
  return passengers.find(p=>name.toLowerCase()===p.name.toLowerCase());
}
...
export function getPassengerIndex(passengers, name) {
  ...
  if(!Array.isArray(passengers)||typeof name!=='string'||name.trim().length===0) return -1;
  return passengers.findIndex(p=>name.toLowerCase()===p.name.toLowerCase());
}
